...
class CSMACDSimulatorGUI:
    ...
    def simular_transmissao(self, nome, rect_id):
        ...
        while self.running:
            ...
            if canal_ocupado:
                self.log(f"{nome}: Canal ocupado, aguardando...")
            ...
            if num_transmitindo > 1:
                ...
                self.log(f"{nome}: Colisão com {num_transmitindo} transmissores!")
                ...
                self.canvas.itemconfig(rect_id, fill="gray")
                self.log(f"{nome}: Backoff {backoff:.2f}s (k={k}, R={R})")
                time.sleep(backoff)
                continue

            # Transmissão bem-sucedida
            self.packet_id += 1
            packet = f"P{self.packet_id}"

            self.status_var.set(f"{nome} transmitindo...")
